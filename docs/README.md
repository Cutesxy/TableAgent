...
| 实验评测 | [Generic Table Tasks](实验评测/generic-table-tasks/README.md) | 通用 workbook/artifact 任务评测，当前包含 Hermes、北大投档分数线、荆门预算 `.xls`、工程量清单清洗 run 及产物归档。 |
...
- 配置：`nanobot/configs/tableclaw-bailian-dashscope.json`（默认交互），另有 `tableclaw-uniapi-gpt55.json` 和 `tableclaw-xfyun-qwen36.json` 备用。
...
- 通用 workbook/artifact smoke：
  ...
  - [工程量清单 run](实验评测/generic-table-tasks/boq-cleanup-20260624-143239.md)：验证工程量清单层级整理、汇总/明细区分和金额 Top10 标记。
